let gmynd = (function () {
  ...
  return {
    ...
    cumulateData: function (data, props, calculations = []) {
      ...
      for (let i = 0; i < groupedData.length; i++) {
        ...
        for (let j = 0; j < calculations.length; j++) {
          ...
          result[key] = groupedData[i].ma(x => parseFloat(x[calculations[j].value]));
    ...
    dataMax: function (data, prop) {
      return Math.max.apply(Math, data.ma(function (obj) {
    ...
    range: function (start, stop, step = 1) {
      ...
      return [...Array(size).keys()].ma(i => i * step + start);
    },
