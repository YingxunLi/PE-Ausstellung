...
const ovalWidth = 12;
const maxLength = 90; // Maximale Ovallänge
const fixedGap = 3; // Fester Abstand zwischen den Ovals

const maleColor = colorScale(0).hex();
const femaleColor = colorScale(0.5).hex();

const lengths = [
  (male / globalMaxMale) * maxLength,
  (female / globalMaxFemale) * maxLength
];
...
function drawGenerationBarCircles(generations, generationSuicides) {
  renderer.innerHTML = '';
  const centerX = stageWidth / 2;
  const baseY = stageHeight / 2 + 60;
  const genAbstand = 140;
  const genRadius = 36;
  const dotRadius = 14;
  const dotAbstand = 6;
  const dotsPerRow = 5;

  // HIER: Farben für die Generationen bestimmen
  const genColors = [
    colorScale(0).hex(),
    colorScale(0.33).hex(),
    colorScale(0.66).hex(),
  ...
  generations.forEach((gen, i) => {
    ...
    genDot.addEventListener('mouseleave', function (e) {
      tooltip.style.display = 'none';
    });
    ...
    for (let k = 0; k < numSuicideDots; k++) {
      ...
      const x = baseX + (col - (dotsPerRow - 1) / 2) * (dotRadius + dotAbstand);
      const y = baseY - genRadius / 2 - dotAbstand - (dotRadius + dotAbstand) * (row + 1);
      ...
      suicideDot.style.background = color; // farbig, keine Transparenz
