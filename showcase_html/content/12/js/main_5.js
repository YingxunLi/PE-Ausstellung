...
function drawCountryGDPRow(countries, countryGDP, minGDP, maxGDP) {
  ...
  allLines.slice(1).forEach(line => {
    ...
    if (year !== Number(yearSlider.value)) return;
...
// Passe diese Werte an deine Daten an:
let sliderMinX = 150; // linker Rand (px, wie CSS left)
let sliderMaxX = 950; // rechter Rand (px)
let sliderMinValue = 1985; // min Jahr
let sliderMaxValue = 2015; // max Jahr
...
function handleMouseMove(event) {
  ...
  let moveOffsetX = moveX - offsetX;

  // Begrenzung auf Slider-Bereich
  if (moveOffsetX < sliderMinX) moveOffsetX = sliderMinX;
  if (moveOffsetX > sliderMaxX) moveOffsetX = sliderMaxX;

  // Position zu Wert umrechnen
  let sliderRange = sliderMaxX - sliderMinX;
  let sliderValueRange = sliderMaxValue - sliderMinValue;
  let sliderValue = Math.round(
    ((moveOffsetX - sliderMinX) * sliderValueRange) / sliderRange + sliderMinValue
  );
  ...
  moveOffsetX = sliderPosition + sliderMinX;
  slider.style.left = moveOffsetX + "px";

  // --- Hier: Visualisierung aktualisieren ---
  yearLabel.textContent = sliderValue;
